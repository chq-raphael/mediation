			...
			<ul>
				...
				<li>
					...
					<?PHP echo $mediationDetail ? $mediationDetail : '暂无'; ?>
						...
						<?php
						if ($receiveState==0 || $state == 1){
						    echo '<div class="mainBtn st2"><a href="'.App_URL.'/index.php?act=index&op=jump&reportId='.$reportId.'">分配调解</a><a href="javascript:window.history.back();">返回</a></div>';
						}elseif ($receiveState==2){
						    if ($mediateState != 0){ //已提交
						        echo '<div class="mainBtn st2"><a href="'.App_URL.'/index.php?act=index&op=pingjia&id='.$mediatorEvaluationId.'">评论</a><a href="javascript:window.history.back();">返回</a></div>';
						    }else{
						        echo '<div class="mainBtn"><a href="javascript:window.history.back();">返回</a></div>';
						    }
						}else{
						    echo '<div class="mainBtn"><a href="javascript:window.history.back();">返回</a></div>';
						}
